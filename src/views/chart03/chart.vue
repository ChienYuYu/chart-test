  ...
  methods: {
    ...
    setPointRadius() {
      this.chartdata.datasets[0].pointRadius = this.CO2Array.map((item, index)=> {
        return index == this.chartdata.datasets[0].data.length -1 ? 7:3
      })
      this.chartdata.datasets[1].pointRadius = this.PM25Array.map((item, index)=> {
        return index == this.chartdata.datasets[1].data.length -1 ? 7:3
      })
    },
    getFatherData() {
      ...
      this.options.scales.xAxes[0].ticks.maxTicksLimit = this.XAxis.length / 10
    },
  },
}
</script>
